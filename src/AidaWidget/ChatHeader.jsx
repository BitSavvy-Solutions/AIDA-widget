...
const ChatHeader = ({
    ...
    const handleSaveTitle = () => {
        if (titleDraft.trim() && onRenameSession) onRenameSession(titleDraft.trim());
        else setTitleDraft(sessionTitle);
    ...
    const handleCreateTag = () => {
        ...
        // ✅ MODIFIED: Pass currentSessionId to handle creation and assignment atomically.
        // This fixes the race condition where projects state was stale for assignment.
            ...
            <div className="flex items-center gap-3 flex-1 min-w-0 mr-2">
                ...
                <div className="flex-1 min-w-0 flex items-center gap-3">
                    {isEditingTitle ? (
                        ...
                        <div className="flex items-center gap-2 min-w-0 group cursor-pointer shrink-0 max-w-[50%]" onClick={() => isSessionActive && setIsEditingTitle(true)}>
                    ...
                    {isSessionActive && (
                        <div className="relative flex items-center" ref={tagMenuRef}>
                            <button
                                ...
                                className={`flex items-center gap-1 p-1 rounded-md transition-colors ${isTagMenuOpen ? 'bg-white/10' : 'hover:bg-white/5'}`}
                                title="Manage Tags"
                            >
                                {assignedProjects.length > 0 ? (
                                    <div className="flex -space-x-1">
                                        {assignedProjects.slice(0, 3).map(p => {
                                            ...
                                            return (
                                                <div key={p.id} className="w-5 h-5 rounded-full border border-[#0f172a] flex items-center justify-center" style={{ backgroundColor: hexToRgba(p.iconColor || DEFAULT_PROJECT_ICON_COLOR, 0.2), borderColor: p.iconColor || DEFAULT_PROJECT_ICON_COLOR }}>
                                                    <Icon className="w-3 h-3" style={{ color: p.iconColor || DEFAULT_PROJECT_ICON_COLOR }} />
                                                </div>
                                            );
                                        })}
                                        {assignedProjects.length > 3 && (
                                            <div className="w-5 h-5 rounded-full bg-gray-700 border border-[#0f172a] flex items-center justify-center text-[9px] text-white">
                                                +{assignedProjects.length - 3}
                                            </div>
                                        )}
                                    </div>
